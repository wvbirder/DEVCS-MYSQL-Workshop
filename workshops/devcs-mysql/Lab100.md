...
In this workshop we will use the toolset now built into Developer Cloud Service. We will assume two personas will be used throughout the labs. Lab 100 will feature the Infrastructure persona (Chip) who is responible for laying down the footprint to support application deployments in the Oracle Cloud. Chip will lay the the ground work for a MYSQL DB running on a Oracle Linux Compute instance. He will build and deploy an environment by creating a "Custom Image" and then create an Oracle Linux VM using that image. This will be accomplished using Developer Cloud Service's Packer and Terraform built-ins. 

The second persona is that of a Developer (Serena) who will deploy two Node.js microservices (AlphaOffice) using Developer Cloud Service in Lab 200 into Oracle Application Cloud Service. The AlphaOffice application will query the MYQL database running on the Compute instance. All of this will be driven through Developer Cloud Service.

In Lab 300 Serena will make a couple of changes in the AlphaOffice UI repository which in turn will fire off a re-build and re-deployment of the AlphaOffice application.
...
You will create some initial required infrastructure components within your account.

## Login to Trial Account

Login with the credentials you have been given. You have most likely already changed the default password for your Trial.
...
**NOTE**: You will be prompted to change the temporary password given to you in the welcome email. In that case, enter your new password into the password field.

### **STEP 3**: Show Cloud Services used in this workshop
...
- **Save off the OCID of the `MYSQL` compartment** by clicking the **Copy** link. **Paste this into a text editor** of your choice as we will be referencing it later on in the labs.
...
- Scroll to the bottom of the page to **hightlight and (right-click) copy** the Tenacy OCID. Save the value in your text editor.

  ![](images/100/18.PNG)

- At the top right of the page you will notice a Region link. The link is not important at this time but the name of the region is. Note what it is and type that into your text editor. In this example it is `us-ashburn-1`.
...
- Copy and paste the **Availibility Domain** for AD-1 and save in your text editor. In this example it's `Yfpl:US-ASHBURN-AD-1`
...
Before you can use Oracle Developer Cloud Service to provision to Oracle Cloud Infrastructure you must configure connections to Oracle Cloud Infrastructure Compute Classic which controls VM builds AND Storage Classic which stores project artifacts.
...
- You'll see on the page a reminder that configuration of  Compute and Storage needs to take place before any builds and deployments can be done.

  ![](images/100/27.PNG)

- Right-click on the **Compute Account** link and select **Open link in new tab**.

  ![](images/100/28.PNG)

- This takes you into an Organization Configuration page where you'll click the **Configure Compute Account** button.

  ![](images/100/29.PNG)

- You'll be asked to supply your **Username**, **Password**, **Service ID** and **REST Endpoint** in the dialog. Enter your login credentials for your Trial Account into the Username and Password fields.

- To obtain the Service ID and REST Endpoint you need to go back to the main services Dashboard, click on the **Compute Classic hamburger menu** and right-click **View Details** to **Open link in new tab**.

  ![](images/100/30.PNG)

- Copy and paste the two values shown here into their respective fields in the dialog.
...
- To obtain the required information for creating the connection to the Storage Classic service you need to go back to the main Dashboard, click on the Storage Classic **hamburger menu**, right-click on **View Details** and select **Open link in a new tab**.

  ![](images/100/34.PNG)

- From the Storage Details page copy the **Storage-xxxxxxxxx** number string portion from the **REST Endpoint (Permanent)** field and paste into the dialog **Service Id** field. Then copy the entire contents of the **Auth V1 Endpoint** field in the Storage Details page into the dialog **Authorization URL** field.

  ![](images/100/35.PNG)

- Add your Trail account **Username** and **Password** and the completed dialog should look something like the example below:

  ![](images/100/36.PNG)

- Click **Create**.

- It will try to make the connection. You can click the **Test Connection** button to verify.

  ![](images/100/37.PNG)

- **To recap, You have created connections to the Compute and Storage Classic services that get used as temporary build services and project artifacts respectively**.

### **STEP 7**: Create Software Template and Virtual Machine configurations
...
- Still within the Organization Administration page click the **VM Templates** link.

  ![](images/100/38.PNG)

- Click **New Template** and enter the following Name. Click **Create**.
...
- Find the **Node.js 6, Terraform, and Packer** software packages. Click the **plus (+)** sign for each one to add it to the template. Click **Done** when all packages have been added.

   ![](images/100/41.PNG)

 - Click the **Virtual Machines** link followed by **New VM**. 

   ![](images/100/42.PNG)

   ![](images/100/43.PNG)

- In the dialog box leave the defaults of **1** and the **PackerTerraformNodeJS** template and click **Add**.

   ![](images/100/44.PNG)

### **STEP 8**: Create the Project

The project we create will contain the builds and deployments for the Infrastructure as well as the AlphaOffice application.

- Click the **Developer Cloud Service** link at the top center of the page.
...
### **Step 9**: Import a GitHub repository and edit the Packer "build.json" file

In this step you will import a GitHub repository into the project and modify a Packer configuration file. Here you will add the various OCIDs that you saved off earlier in your text editor into a configuration file within the repository.

- In the Repositories section click **New Repository**.
...
- Select the **Import existing repository** radio button and then type OR cut and paste the following:
...
- You will see the repository contents displayed. Click on the **oci_api_key_public.pem** file. This a public API key required for REST API access to our Compute Service. You will copy and paste the contents into your Compute Service User. 

   ![](images/100/51-5.PNG)

- Select the entire contents of the API Public Key and right-click **copy**.

   ![](images/100/51-6.PNG)

- Back in the main Dashboard click on the Compute Service **hamburger menu**, right-click on **Open Service Console** and select **Open link in new tab**..
...
- Click **Add Public Key**. **Paste** the contents of the `oci_api_public_key.pem` file you copied in the Developer project. Click **Add**.

   ![](images/100/51-10.PNG)

- A new PK Fingerprint will appear.

   ![](images/100/51-11.PNG)

**NOTE:** This fingerprint should match the Fingerprint stored in the `build.json` file.

- Leave the Compute Sevice tab open as we will be coming back to it later. 

- Now, **Back in the Developer project** click on the root directory **slash /** to see all of the files in the `MYSQL-PackerTerrform.git` repository.
...
**NOTE: This is where you will be copying those values you saved off earlier in the lab. It's importent to match the proper value with the environment variable. Tenany OCID to Tenacy, User OCID to User, etc**.
...
- **Replace the 5 variables above** in the file where the current values are **XXXX-HERE** with the corresponding values you saved in the text editor. You can paste these values into the file using CRTL-V.
...
```
"region": "<your-region>"
"base_image_ocid": "<The OCID of the base image associated with your region. The comments section at the top of the file contains the default values... the current default is for the ASHBURN region>"
```

- Click the **Commit** button. Also click **Commit** at the pop up dialog prompt.

   ![](images/100/56.PNG)

### **Step 10**: Build the Packer job

In this step you will define a Packer build using the Software Template you created in Step 7. This will build and create a Custom Image with MYSQL installed into your Compute Service.

- From the project menu click the **Build** link.
...
-- Under the Source Control tab select **Git** and then choose the **MYSQL-PackerTerraform.git** repository.

   ![](images/100/60.PNG)

   ![](images/100/61.PNG)

- Select the **Builders** tab and choose **Unix Shell Builder**.

   ![](images/100/62.PNG)

- **Cut and Patse** the following into the Script section:
...
The script fires off a Packer build based on the `build.json` contents and then places the OCID of the new Custom Image into an environment variable which can be referenced from other builds.

- Select the **Post Build** tab and add the **Artifact Archiver** Post Build Action. Enter the following into the **Files to Archive** field:
...
- Go back to the Compute Service Console and click on the **hamburger menu**. Select **Compute-->Custom Images**.

   ![](images/100/70.PNG)

- On a successful build you should see a new Custom Image called `DevCS_MYSQL`. Click the **Show** link to expand the OCID. The OCID will match the OCID in the build log.

   ![](images/100/71.PNG)

   ![](images/100/72.PNG)

### **Step 11**: Edit the Terraform configuration file

In this step you will edit Terraform config file.

- You already have the baseline Terraform files in the imported Git repository. You still need to modify a few of the environment variables to match your specific Tenancy OCIDs as we did in the previous Packer build. Back in the Developer project click on the **Code** link, make sure the **MYSQL-PackerTerraform.git** repository is selected. Click on the **env-vars** file. 
...
- The following values will need to be typed OR **copied and pasted** from the text editor into your `env-vars` file:

    - **TF_VAR_user_ocid**
    - **TF_VAR_tenancy_ocid**
    - **TF_VAR_compartment_ocid**
    - **TF_VAR_region** (us-ashburn-1 is the default)

- **Fill in the 4 variables above** with the corresponding values you saved earlier. You can paste into the file using CRTL-V.
...
**NOTE: If your region is different than the default `us-ashburn-1` also enter that value**.

- Click the **Commit** button. Also click **Commit** at the pop up dialog.

   ![](images/100/56.PNG)

### **Step 12**: Build the Terraform job
...
- Select the **Build Environment** tab and from the dropdown choose **Copy Artifacts**
...
This script substitutes the client_image OCID generated from the Packer build and populates that field in the `env-vars` file. Then it runs Terraform commands to create a new Compute instance based on the Custom image and displays the Public IP address of the Compute instance.
...
- Click the **Build** tab in the Developer project.

   ![](images/100/85.PNG)

- Click on the **Pipelines** tab, then click **New Pipeline**. In the dialog enter:
...
### **Step 13**: (**OPTIONAL**) Execute the Pipeline

This is an optional step which will create the Custom Image and Compute instance in one sequentail build. Typically, there would be a clean up "build" in front of this flow to delete the older versions. That is out of the scope of this lab but if you'd like to execute the Pipeline you can manually delete the Compute instance and Custom Image first. If you don't delete them first you'll get another Custom Image AND Compute instance with unique OCID's even though the displayed names would be the same.
...
### **STEP 14**: SSH into the `MYSQL` Compute Instance

In this step you will SSH into the Compute instance and verify the MYSQL installation. Here, we will show two methods to SSH into the instance; Putty (for Windows based clients) and Linux.

- If you need Putty and/or PuttyGen you can download from here: - [Putty Download](https://www.ssh.com/ssh/putty/download)

- In the Developer project click on the **Code** link and in the `MYSQL-PackerTerraform.git` repository click on the **oci_instance_key** file. Select the entire contents of the file and right-click **Copy**.

- **Save the contents into a file on your local laptop**. (Example: You can save it as openssl.key)

   ![](images/100/100-1.PNG)

   ![](images/100/100-2.PNG)

- We need to convert the OpenSSL format version of the oci_instance_key to PPK. If using Windows **start PuttyGen** and **Load** the file you just saved. 

   ![](images/100/100-3.PNG)

- Save the imported key in PPK format by clicking **Save Private Key**.

   ![](images/100/100-4.PNG)

- Save the file as: (**NOTE:** The .ppk suffix will be added)
...
- **Start Putty** and select the **Session** section and type in the Public IP address of the `MYSQL` Compute instance.
...
- Select **SSH-->Auth** and browse to the Private Key you just saved (`MYSQL.ppk`):

   ![](images/100/103.PNG) 

- Click the **Open** button. You will presented the first time with an alert message. Click **Yes**

   ![](images/100/104.PNG)

- You will be logged into the `MYSQL` Compute image. Type the following cmds:
...
- This will confirm that the build configured the firewall port (3306) for access to the MYSQL database.

   ![](images/100/105.PNG)

- **NOTE: For Linux and Mac client sessions** "cd" into the directory where you saved the `oci_instance_key` file. Make sure the private key file has the permissions of "600" (chmod 600 <your-name-of-the-file>) and ssh into the compute instance `substituting your IP address`.
...
chmod 600 MYSQL.key
ssh -i ./MYSQL.key opc@129.213.137.223
```
...
### **STEP 15**: Verify the MYSQL installation
...
- From your Putty or Lixux/Mac ssh session from Step 14 type the following:
...
- ... to end the Putty, Linux/Mac ssh session.

**You are now ready to proceed to Lab 200**
