...
- Click **Show** on the following services if they are not already seen on your screen:
    ...
    - **Autonomous Developer** OR **Developer**
...
- Gather Tenancy OCID to be used by clicking the **hamburger icon** in the upper left corner to open the navigation menu. Under the Administration section of the menu, click **Tenancy Details**. editor.

  ![](images/100/17-2.PNG)

- In the details page click the Copy link of the OCID. Save that value in your text editor.
...
- Select your compartment from the LOV.
  ...
  **NOTE: You may have to refresh your browser page to see the Compartment you just created.**
...
- You'll breifly see a message saying the credentials have been successfully saved. On the left hand side of the page. Right-click the **Storage** link and select **Open link in a new tab**.
...
- So, for example:
  ...
  **NOTE: The Region `us-ashburn-1` is shown in this example**.
...
- In the Developer project click on the **Code** link and in the `MYSQL-PackerTerraform.git` repository click on the **oci_instance_key** file. Select the entire contents of the file and right-click **Copy**.
  ...
  **`NOTE: If you are using Linux or Mac client then go to the instructions near to bottom of this STEP`**
...
- This will confirm that the build configured the firewall port (3306) for access to the MYSQL database.
  ...
  **NOTE: For Linux and Mac client sessions**:
   "**cd**" into the directory where you saved the `oci_instance_key` file. Make sure the private key file has the permissions of "600" **(chmod 600 \<name-of-your-private-key-file\>)** and **ssh** into the compute instance `substituting your IP address`.
