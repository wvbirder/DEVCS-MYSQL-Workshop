...
# Deploy AlphaOffice Application
...
- Select the **Import exsiting repository** radio button and enter OR **Copy and Paste**:
...
- **The deployment connects to the Application Container Cloud Service in your Trial Account and deploys the application into a container** (Docker in this case). It may be several minutes before it is completed.
  ...
  **ON rare occasions you may see a Gateway Timeout (error 504) but the deployment actually finishes. You can refresh the browser page at this point to verify. Clicking the right hand Blue Bar shows history of deployments, start/stops, etc.**

  ![](images/200/42-6.PNG)

- **`Create Succeeded indicates deployment finished OK.`**

  ![](images/200/42-8.PNG)

- **NOTE: If you still see an error on the first Start or Deployment do a Redeploy**. (example shown)
...
- You can double check the deployment inside the Application Container Cloud Service Console by going to the main Dashboard and clicking on the Application Container **hamburger menu**, right-clicking on **Open Service Console** and selecting **Open link in new tab**.
...
- Back in the Project Deploy screen right-click on `AlphaOfficeUI` and select **Open link in new tab**. It should show the AlphaOffice application displaying the Product Catalog obtained from the MYSQL database.
