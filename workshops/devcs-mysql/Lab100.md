...
### **Step 7**: Import a GitHub repository and add API Key to OCI User
...
- Under `API Keys` click **Add Public Key**. **Paste** the contents of the `oci_api_public_key.pem` file you copied in the Developer project. Click **Add**.

   ![](images/100/51-10.PNG)

- A new PK Fingerprint will appear. **Copy and Paste** this Fingerprint value to the text editor where you have been saving OCID's, etc.

   ![](images/100/51-11.PNG)

- Back in the Developer Project click on the **oci_api_key.pem** file. **Copy and Paste** the contents of this file into your text editor where you are saving the other values. We will reference this value in the next step.

   ![](images/100/51-12.PNG)

   ![](images/100/51-13.PNG)

- Leave the Compute Sevice tab open as we will be coming back to it later. 

### **Step 8**: Configure the DevCS connection to your OCI account
...
- In the `Configure OCI Account` dialog you will be Copying and Pasting the values you have saved in your text editor for:
   ...
   - **Private Key:** Here is where you Paste the contents of the **oci_api_key.pem** file
   - **Fingerprint** 
   - **Compartment OCID**
   - **Storage Namespace**

- When all values are in place click the **Validate** button to confirm. If successfull, click **Save**.

  ![](images/100/27-4.PNG)

### **STEP 9**: Create Software Template and Virtual Machine configurations
...
### **Step 10**: Edit the Packer "build.json" file
...
### **Step 12**: Edit the Terraform configuration fil

In this step you'll retrieve a unique Key Pair and save those values into your text editor. Thpublic key of the pair will be used to create a Conpute Instance based on the Custom Image you just created in the previous step. The private key will be used later on in the lab when you SSH into the Compute Instance during final testing. A key pair generator has been staged in a Fn Server. You will use curl to retrieve the key pair.

- From your local laptop/server open up a Command Prompt (Windows or Terminal session (Linux/Mac) and obtain a unique key pair. **Copy and Paste** the following URL:

  ```
  curl http://129.213.127.7:8080/t/genkeys/genkeys > mykeys
  ```

- A Key Pair will be returned: 

  **Example (Windows Notepad will be shown):**

  ![](images/100/72-2.PNG)

  **NOTE:** There are three different values returned.
    - The first part is the PRIVATE key in PEM format
    - The second part is the PUBLIC key in PEM format
    - The last part is the PUBLIC key in OpenSSH format

  **For this lab we will be using the first and last values.**

- Open the `mykeys` file in a text editor. **Copy and Paste** the PRIVATE KEY portion into the **oci_instance_key** file located in the **MYSQL-PackerTerraform.git** repository by doing the following:

  - From your text editor select only the PRIVATE key value and then right-click **Copy**:
...
- Now repeat the sub-steps above to Copy the Public OpenSSH format key into the **oci_instance_key.pub** file in the Git repository:

  - From your text editor select only the OpenSSH PUBLIC key value and then right-click **Copy**:

    ![](images/100/72-11.PNG)
...
- You already have the baseline Terraform files in the imported Git repository. You still need to modify a few of the environment variables to match your specific Tenancy OCIDs as we did in the previous Packer build. Go back into the Developer project and click on the **Code** link, make sure the **MYSQL-PackerTerraform.git** repository is selected. Click on the **env-vars** file. 
