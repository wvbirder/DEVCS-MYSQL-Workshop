...
- Now change the the URL to query one product. Change the URI to:
...
- **Navigate to target-->js**. Click on the **AlphaOffice.js** file and then click on the **pencil icon** to edit. You are going to change the **dbServiceURL** variable and replace the **localhost:8002** portion of the string with the URL of the newly deployed `RESTClient` Application Container. To obtain that URL click on the project **Deploy** link and right-click on the AlphaOfficeREST link and select **Copy link address**. 

  ![](images/200/35.PNG)

- Once copied go back to the Code repository edit session and replace the current string with the copied URL. In the project Code section of the AlphaOfficeUI.git **Navigate to target-->js**. Click on the **AlphaOffice.js** file and then click on the **pencil icon** to edit. You can use CTRL-V to paste the string into the file.
...
- Back in the Developer project Deploy screen right-click on the `AlphaOfficeUI` deployment and select **Open link in new tab**. It will show the AlphaOffice application.
