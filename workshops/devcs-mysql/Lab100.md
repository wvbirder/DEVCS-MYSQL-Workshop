...
### **Step 16**: (**OPTIONAL**) Execute the Pipeline
...
### **STEP 17a**: SSH into the `MYSQL` Compute Instance

In this step you will SSH into the Compute instance and verify the MYSQL installation. Here, we will show two methods to SSH into the instance; Putty (for Windows based clients) and Linux/Mac.

**`NOTE: If you are using Linux or Mac client then go to STEP 17b`**

- If you need Putty and/or PuttyGen you can download from here: - [Putty Download](https://www.ssh.com/ssh/putty/download)

- In the Developer project click on the **Git** link and in the `MYSQL-PackerTerraform.git` repository click on the **oci_instance_key** file. Select the entire contents of the file and right-click **Copy**.
...
- Save the imported key in PPK format by clicking **Save Private Key**. If prompted that you are saving without a Passphrase that is fine. we will save without a Passphrase.
...
- **Start Putty** and select the **Session** section and type in the Public IP address of the `MYSQL` Compute Instance. This IP address is in the Details page:

  Example:

   ![](images/100/100-6.PNG)

- Use the IP adress in the **Putty Session-->Host Name** field.

   ![](images/100/101.PNG)

- Select the **Data** section and enter the following as the **Auto-login username**:
...
### **STEP 17b**: SSH into the `MYSQL` Compute Instance
...
- **Open a terminal session save the contents into a file**.   
 
  (Example: You can save it as MYSQL.key)

- Make sure this private key file has the permissions of "600" **(chmod 600 \<name-of-your-private-key-file\>)**
...
- Find the Public IP address found in the Details page of the MY_SQL Compute Instance:
...
- SSH into the instance using the following command:
  ...
  ```
  ssh -i ./MYSQL.key opc@<PUBLIC IP>
  ```

  Example: `ssh -i ./MYSQL.key opc@129.213.17.80`
...
### **STEP 18**: Verify the MYSQL, Git and Docker installations
...
- From ether your Windows Putty or Lixux/Mac SSH session **Type** the following:
...
- To exit your Putty or Terminal session **Type**:
