...
- From the Storage Details page copy the **Storage-xxxxxxxxx** number string portion from the **REST Endpoint (Permanent)** field and paste into the dialog **Service Id** field. Then copy the entire contents of the **Auth V1 Endpoint** field in the Storage Details page into the **Authorization URL** field.
...
- **To recap, You have created connections to the Compute and Storage Classic services that get used as temporary build services and project artifacts respectively going forward**.

### **STEP 7**: Create Software Template and Virtual Machine configurations

In this step Software Templates define what supporting software packages get incorporated into the build servers used within Developer Cloud Service projects. The Virtual Machine definitions define the number of VMs based on one of the Software Templates.
...
In this step you will import a precreated repository into the project and modify a Packer configuration file. Here you will add the various OCIDs that you saved off earlier in the lab into a configuration file.
...
- You will see the repository contents displayed. Click on the **oci_api_key_public.pem** file as we are going to copy and paste this into our Compute Service user. 
...
- Back in the main Dashboard click on the Compute Service **hamburger menu**, right-click on **Open Service Console**.
...
- Click the **Add Public Key link**. **Paste** the contents of the oci_api_public_key.pem file you copied in the Developer project. Click **Add**.
...
- Leave this tab open as we will be coming back to it later. 

- Now, **Back in the Developer project** click on the root directory slash **/** to see all of the files in the local repository.
...
- **Replace the 5 variables above** in the file where the current values are **XXXX-HERE** with the corresponding values you saved off into your text file. You can paste into the file using CRTL-V.
...
Becomes: **NOTE:** The values stay in the double quotes
...
```
region: "<your-region>"
base_image_ocid: "<the OCID of the base image associated with your region in the comments section at the top of the file... the default is ASHBURN>"
```
...
In this step you will define a Packer build using the Software Template you created in Step 7. This will build and create a Custom Image with MYSQL installed into your Trial Account.
...
The script fires off a Packer build based on the build.json contents and then places the OCID of the new Custom Image into an environment variable which can be referenced from ant other builds.
...
- Go to the Compute Service Console and click on the **hamburger menu**. Select **Compute-->Custom Images**.
...
- You already have the baseline Terraform files in the imported Git repository. You still need to modify a few of the environment variables to match your specific Tenancy OCIDs as we did in the previous Packer build. Click on the **Code** link, make sure the **MYSQL-PackerTerraform.git** repository is selecting the click on the **env-vars** file. 
...
- The following values will need to be typed OR **copied and pasted** into your env-vars file:
...
- **Fill in he 4 variables above** with the corresponding values you saved off into your text file. You can paste into the file using CRTL-V.
...
Becomes: **NOTE:** The `TF_VAR_client_image` variable remains empty as it will be populated during the build.
...
In this step you will create and execute the Terraform build job to create a Compute instance based on the Custome Image created in the Parcker build.
...
This script substitutes the client_image OCID generated from the Packer build and fills in that field in the env-vars file. Then it runs Terraform commands to create a new Compute instance based on the Custom image and displays the Public IP address of the Compute instance.
...
- Click on the **Build** tab in Developer project.
...
- Drag and Drop the **MYSQL_Packer** and **MYSQL_Terraform** builds onto the canvas and then **connect them together** along with the **Start** icon.
...
This is an optional step which would create the Custom image Compute instance and one sequentail build. Typically, there would be a clean up build in front of this flow to delete the older versions. That is out of the scope of this lab but if you'd like to execute the Pipeline you can manually delete the Compute instance and Custom Image first. If you don't delete them first you'll get another Custom Image AND Compute instance will unique OCID's even though the displayed names would be the same.
...
- It will several minutes but both builds to complete. Progress is updated as the flow proceeds.

   ![](images/100/96.PNG)

- When the build are completed you can click on the **View Recent Build History** link to verify status. 

   ![](images/100/97.PNG)

   ![](images/100/98.PNG)

- You can go back into the Compute Cloud Service Console and see the newest deployments of the `DevCS_MYSQL` Custom Image and the `MYSQL` Compute instance.
...
- **Save the contents into a file on your local laptop** (In this example the file name will be dockerkey.ppk.

   ![](images/100/100-1.PNG)

   ![](images/100/100-2.PNG)

- We need to convert the OpenSSL format version of the oci_instance_key to PPK. If using Windows bring up PuttyGen and **Load** the file you just saved. 
...
- Give the filename ans save the file: (**NOTE:** The .ppk suffix will be added)
...
- This will confirm that the build.
...
- From your Putty terminal session from Step 14 type the following:
...
- You should see output as shown below proofing out the installtion and configuration of MYSQL and the AlphaOffice database schema.
